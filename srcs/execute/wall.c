...
/*   Updated: 2023/08/16 19:02:58 by wonlim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(int direction, t_var v, t_data *d, int x)
{
	...
	v.step = 1.0 * texture.height / v.wall_height;
	tex_pos = (v.draw_start - HEI / 2 + v.wall_height / 2) * v.step;
	y = v.draw_start;
	while (y < v.draw_end)
	{
		tex_y = (int)tex_pos & (texture.height - 1);
		tex_pos += v.step;
		d->image.buffer[x + y * WID]
			= texture.buffer[texture.height * tex_y + tex_x];
		y++;
	}
}
