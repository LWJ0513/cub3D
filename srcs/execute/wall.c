...
/*   Updated: 2023/08/08 17:44:50 by wonlim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	v->wall_height = (int)(d->height / v->perp_wall_dist);
	v->draw_start = -v->wall_height / 2 + d->height / 2;
	if (v->draw_start < 0)
		v->draw_start = 0;
	v->draw_end = v->wall_height / 2 + d->height / 2;
...
void draw_wall(int direction, t_var v, t_data *d, int x)
{
	...
	tex_pos = (v.draw_start - d->height / 2 + v.wall_height / 2) * step;
	...
	while (y < v.draw_end)
	{
		...
		d->image.buffer[x + y * d->width] = color;
		y++;
	}
}
